...
class CalculateMealNutrients(
    ...
    private fun dailyCaloriesRequirement(userInfo: UserInfo): Int {
        ...
        val goalMultiplier = when (userInfo.goalType) {
            is GoalType.LoseWeight -> 0.9f
            is GoalType.KeepWeight -> 1f
            is GoalType.GainWeight -> 1.1f
        }

        return (bmr(userInfo) * activityFactor * goalMultiplier).roundToInt()
    }
